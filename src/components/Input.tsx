...
let Input = ({ label, value, onChangeText, placeholder, error, secureTextEntry, style, keyboardType, readonly, searchPadding, icon, formData, phoneNumberPadding, maxLength, autoCapitalize, autoCorrect, inputStyle }: InputProps) => {
  ...
  const openDrawer = () => {
    ...
    if (formData) {
      ...
      if (!error) {
        const calculatedEMISchedule = calculateEMISchedule(calculationFieldObj["actualPrice" as any], calculationFieldObj["prepaidPrice" as any], calculationFieldObj["installmentType" as any], calculationFieldObj["noOfEmis" as any], calculationFieldObj["emiDate" as any])
  ...
  // console.log('ME52RETAILERTESTING',"We are input yayyyy!!")
    ...
    <KeyboardAvoidingView style={[style]} behavior={Platform.OS == "ios" ? 'padding' : undefined}>
      ...
      >
        <BottomSheetScrollView style={{ paddingVertical: 15, paddingHorizontal: 20, height: 600 }}>
          ...
          {bottomError ? (<View style={{ flex: 1, justifyContent: 'center' }}>
            <Text style={{ color: colors.text, alignSelf: 'center', justifyContent: 'center' }}>Please check Actual Price, Down Payment, I  nstallment Type, EMI start date and Number of installement is fill</Text>
          </View>) :
            (
              ...
              //     renderItem={({item: bottomSheetD, index}) => (
